...
namespace StudensManagement_HW_Q5_10
{
    public partial class ActionsControl : UserControl
    {
        ...
        //public FilterDelegate GradeFilter = (Student student) => 
        //{
        ...
        public ContainDelegate moreThan50Delegate = (Student student) => { return student.Grade > 50; };


        // function for question 8

        public ContainDelegate sameFirstLetter = (Student student) => { return student.FirstName[0] == student.LastName[0]; };


        // functions for question 9

        public SumDelegate TotalTuitionPaid = (Student student) => { return student.TiutionPaid; };

        public SumDelegate TotalTuitionLeft = (Student student) => { return student.TiutionLeft; };


        // functions for question 10

        public SumDelegate TotalAges = delegate (Student student) { return student.Age; };
        
        public SumDelegate GetTotalGrades = (Student student) => { return student.Grade; };

        public AverageDelegate GetAverage = (int sum, int numOfStudents) => { return sum / numOfStudents; };
        ...
        private async void button2_Click(object sender, EventArgs e)
        {
            switch (comboBox2.Text)
            {
                case "Grade more than 50":
                    if (StudentList.Contain(moreThan50Delegate))
                    {
                ...
                case "Same first letter":
                    if (StudentList.Contain(sameFirstLetter))
                    {
                        pictureBox1.Visible = true;
                    }
                    else
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            switch (comboBox3.Text)
            {
                ...
                case "Ages":
                    textBox4.Text = StudentList.GetAverage(TotalAges, GetAverage).ToString();
                    break;
